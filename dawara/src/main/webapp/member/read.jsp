...
<DIV class='content'>
  ...
  <FORM name='frm' id='frm' method='POST' action='./update.do' 
    ...
    <table>
      <tr>
        <td style='padding-left: 135px;'><label for="mem_job" class="col-md-2 control-label" style='width: 100%;'>직업</label></td> 
    ...
    <div class="form-group">
      <label for="address2" class="col-md-2 control-label">회원 이미지</label>    
      <div id='file1Panel' class="col-md-10">
      <!-- EL 값을 JSTL 변수에 할당 -->
              <c:set var='mem_pic' value="${fn:toLowerCase(memberVO.mem_pic)}" />
              <c:choose>
                <c:when test="${fn:endsWith(mem_pic, '.jpg')}">
                  <IMG id='mem_pic' src='./storage/${memberVO.mem_pic}' style='width:150px; height:130px;'>
                </c:when>
                <c:when test="${fn:endsWith(mem_pic, '.gif')}">
                  <IMG id='mem_pic'  src='./storage/${memberVO.mem_pic}' style='width:150px; height:130px;'>
                </c:when>
                <c:when test="${fn:endsWith(mem_pic, '.png')}">
                  <IMG id='mem_pic'  src='./storage/${memberVO.mem_pic}'' style='width:150px; height:130px;'>
                </c:when>
              </c:choose>
      </div>
    ...
    <div class="form-group">
      <label for="fileMF" class="col-md-2 control-label">회원 이미지 수정</label>    
      <div class="col-md-10">
        ...
        <h6 style='color: red;'>&nbsp;&nbsp;&nbsp;(사진 수정 시, 반드시 사이즈 확인 버튼을 눌러주세요.)</h6>
      </div>
    </div>
