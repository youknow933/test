...
VALUES((SELECT nvl(MAX(mem_no), 0) + 1 as mem_no FROM member), 1, 'schyyy', '123', 'SCH', '01022220000', 'schyyy@naver.com', '학생', 0, 'Y', SYSDATE);
INSERT INTO member(mem_no, auth_no, mem_id, mem_pass, mem_name, mem_tel, mem_mail, mem_job, mem_acc, mem_confirm, mem_date) 
VALUES((SELECT nvl(MAX(mem_no), 0) + 1 as mem_no FROM member), 1, 'BUYBUYBUY', '123', 'BUY', '01033330000', 'buy@naver.com', '학생', 0, 'Y', SYSDATE);
...
INSERT INTO deal(deal_no, d_state, d_date, mem_no, pr_no)
VALUES ((SELECT nvl(MAX(deal_no), 0) + 1 as deal_no FROM deal), '거래중', '택배거래', SYSDATE-1, 3, 1);

INSERT INTO deal(deal_no, d_state, d_date, mem_no, pr_no)
VALUES ((SELECT nvl(MAX(deal_no), 0) + 1 as deal_no FROM deal), '거래중', '택배거래', SYSDATE-2, 2, 2);

INSERT INTO deal(deal_no, d_state, d_date, mem_no, pr_no)
VALUES ((SELECT nvl(MAX(deal_no), 0) + 1 as deal_no FROM deal), '거래중', '택배거래', SYSDATE-3, 4, 3);

INSERT INTO deal(deal_no, d_state, d_date, mem_no, pr_no)
VALUES ((SELECT nvl(MAX(deal_no), 0) + 1 as deal_no FROM deal), '거래중', '택배거래', SYSDATE-3, 4, 4);

----------------------------------------------------------------------------------------

2) 목록
select DEAL_NO, D_STATE, D_CHECK, D_DATE, mem_no, PR_NO from deal;
...
-- 택배거래 조회
select DEAL_NO, D_STATE, D_CHECK, D_DATE, mem_no, PR_NO 
from deal
WHERE D_WAY = '택배거래';
...
FROM deal;
 ...
 DEAL_NO D_STATE D_WAY D_CHECK D_DATE                MEM_NO PR_NO
 ------- ------- ----- ------- --------------------- ------ -----
       3 거래진행중   택배거래  N       2018-07-01 16:38:35.0      4     3
       1 거래중     택배거래  N       2018-07-03 16:38:33.0      3     1
       2 거래중     직거래   N       2018-07-02 16:38:34.0      2     2
...
ORDER BY d.deal_no;
 ...
 ----- --------------------------- -------- --------- ------ ------- ---------- -------- ------- -------- -----
     1 [삼성전자]PEN NT930QAA-K28A 800              1 SCH          1 거래중     택배거래 N              3 NNN
     2 [LG] 2018 그램 14인치       800              1 SCH          2 거래중     직거래   N              2 YYY
     3 [DELL] XPS 13               800              1 SCH          3 거래진행중 택배거래 N              4 BUY
...
select * from product;
 ...
 ----- ------- ------ ----------------------- ----------- --------- -------- -------- ------ --------------------- --------------------- -------- -------------- -------- ----------- -------- ---------- ----------
     3       1      1 [DELL] XPS 13           크리스마스에 갔던곳  NULL      NULL     NULL          0 2018-07-04 16:35:39.0 2017-01-01 00:00:00.0 중고       직거래            서울/종각           1000      800 Y          N
     4       1      1 [MS] 서피스랩탑2             크리스마스에 갔던곳  NULL      NULL     NULL          0 2018-07-04 16:35:55.0 2017-01-01 00:00:00.0 중고       직거래            서울/종각           1000      800 Y          N
     1       1      1 [삼성전자]PEN NT930QAA-K28A 크리스마스에 갔던곳  NULL      NULL     NULL          0 2018-07-04 16:34:52.0 2017-01-01 00:00:00.0 중고       직거래            서울/종각           1000      800 Y          N
     2       1      1 [LG] 2018 그램 14인치       크리스마스에 갔던곳  NULL      NULL     NULL          0 2018-07-04 16:35:15.0 2017-01-01 00:00:00.0 중고       직거래            서울/종각           1000      800 Y          N
...
-- 거래 수정      
UPDATE deal
SET d_state='거래완료'='택배거래', d_check='Y'
WHERE deal_no = 12
