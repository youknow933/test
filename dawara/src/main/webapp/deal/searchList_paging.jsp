...
<DIV class='content'>
  ...
  <FORM name='frm_delete' id='frm_delete' method='post' action='./delete.do'>
    <input type='hidden' name='deal_no' id='deal_no' value='${param.deal_no }'>
  ...
  <c:choose>
    <c:when test="${sessionScope.mem_no eq '' or sessionScope.mem_no eq null}">
      <H2 class="h2underline">모든 거래 목록(관리자)</H2>
    </c:when>
    <c:otherwise>
      <H2 class="h2underline">${sessionScope.mem_id }(${sessionScope.mem_name })님의 직거래 목록</H2>
    </c:otherwise>
  </c:choose>
  
  <form name="frm" id="frm" method ='GET' action="./searchList_paging.do" >
    <input type='hidden' name='nowPage' id='nowPage' value='${param.nowPage }'>
...
<!--       <span class="menu_divide"> | </span> -->
      <button type="button" onclick="location.href='./searchList_paging.do'">전체 보기</button>
    </aside>
    <br>
    <div class="searchbox">
      <div class="bloc">
  ...
  <tbody>
    ...
    <c:forEach var="Member_Product_DealVO" items="${list }">
      <TR>
        ...
        <TD>
          <A href="./read.do?deal_no=${Member_Product_DealVO.deal_no }">${Member_Product_DealVO.pr_title}</A>
        </TD>
        <TD style='text-align: center ;'>${Member_Product_DealVO.pr_price}원</TD>
        <TD style='text-align: center ;'>${Member_Product_DealVO.seller}<em class="fieldName">(${Member_Product_DealVO.seller_no})</em> </TD>
        <TD style='text-align: center ;'>
          <A href="javascript:update(${Member_Product_DealVO.deal_no })"><span class="glyphicon glyphicon-edit"></span></A>
