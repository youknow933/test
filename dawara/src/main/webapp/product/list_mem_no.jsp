...
<body>
  <DIV class='container'>
    ...
    <SPAN style="font-size: 20px">내가 판매중인 물품 </SPAN>
      ...
      <table class="table table-striped" style='width: 100%;'>
        ...
        <tbody>
          <c:forEach var="productVO" items="${list }">
            <c:choose>
              <c:when test="${productVO.pr_visible == 'Y'}">
                <tr>
                  <td style='vertical-align: middle;'>
                    <c:choose>
                      <c:when test="${productVO.pr_thumb != ''}">
                        <IMG id='pr_thumb' src='./storage/${productVO.pr_thumb}'>
                        <!-- 이미지 파일명 출력 -->
                      </c:when>
                      <c:otherwise>
                        <IMG id='pr_thumb' src='./images/none1.jpg' style='width: 120px; height: 80px;'>
                        <!-- 파일이 존재하지 않는 경우 -->
                      </c:otherwise>
                    </c:choose>
                  </td>
