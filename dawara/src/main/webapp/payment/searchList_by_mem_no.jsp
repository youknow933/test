...
<DIV class='content'>
  ...
  <c:choose>
    <c:when test="${sessionScope.mem_no eq null}">
      <H2 style='line-height: 48px; border-bottom: 2px solid #383838;'>모든 결제내역 List</H2>
    </c:when>
    <c:otherwise>
      <H2 style='line-height: 48px; border-bottom: 2px solid #383838;'>회원 결제내역 List</H2>
    </c:otherwise>
  </c:choose>
...
<%--       <a href="../product/list_by_category.do?gory_no=${param.gory_no }">판매 목록</a><!-- 개발 중 . . . --> --%>
      ...
      >
      <a href="../shipping/searchList_by_mem_no_paging.do">택배거래 List</a>
      >
      <a href="./searchList_by_mem_no.do">결제내역 List</a>
      <!-- 검색 -->
